...
`launch.sh` automatically; manually started warp panes are adopted within
seconds by the scanner. The watcher exits and releases authority when the
pane dies, when warp exits (launch.sh and the `exit` subcommand both release
immediately), or after the warp chrome has been absent for a while (e.g.
shell after exit, or a full-screen command like `vim`).

Note: releases (like reports) must carry a fresh monotonically increasing
`--seq`; herdr silently ignores a release whose sequence is older than the
last report's, which shows up as the agent record never leaving
`herdr agent list`.
